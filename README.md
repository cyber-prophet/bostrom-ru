...
Cyb.ai - это веб-интерфейс, который разрабатывается командной Bostrom. В числе многих функций Cyb.ai:

- получает данные о киберлинках записанных blockchain Bostrom из промежуточной базы данных
- отправляет транзакции в Bostrom о создании киберлинков (подписанные от имени конкретного адреса в расширении keplr)
- получает и отрпавляет объекты в IPFS при помощи встроенной библиотеки [ipfs-js](https://github.com/ipfs/js-ipfs)
...
Cyberlink - это два и более связанных particle зарегистрированных в Bostrom от имени конкретного нейрона. Киберлинки имеют направление из particle (частицы) отправления в частицу назначения. 

Если particle указан в качестве назначения, значит что у него есть входящие киберлинки. Если particle указан как отправление то у него есть исходящие киберлинки. 

В случае, если на hash текстового файла конкретного поискового запроса не создано cyberlinks, то в выдаче отображаются объекты прилинкованные к хешу текстового файла с содержимым "0".
