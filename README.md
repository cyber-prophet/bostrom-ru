...
В сети Bostrom действуют следующие токены: BOOT, HYDROGEN, VOLT, AMPERE. Все токены имеют прикладное значение. 
...
Bostrom устроен так, чтобы экономически инцентивизировать валидаторов и держателей токенов (neuron, нейронов в терминах Bostrom) выполнять полезные для системы функции.
...
- использовать токен H (HYDROGEN, ВОДОРД), который получается в обмен на `staked` BOOT, для инвестминтинга A и V токенов (и использовать их для создания сyberlinks, т.е. обучения Bostrom).
