...
### Механизм работы cyb.ai 

[Cyb.ai](https://cyb.ai) - это веб-интерфейс, который разрабатывается командной Bostrom. В числе многих функций Cyb.ai:
...
В cyb.ai есть поисковая строка. Когда пользователь вводит в поисковую строку запрос cyb.ai вычисляет ipfs хеш текстового файла с содержимым поискового запроса. Далее, cyb.ai делает запрос к базе данных, в которой реплицируются данные о записанных в Bostrom киберлинках. Если в базе данных находятся cyberlinks исходящие из ipfs хеша поискового запроса, то в ответ возвращаются хеши конечных файлов отсортированные по cyberrank.

Таким образом: результаты выдачи в cyb.ai это файлы, хранящиеся в IPFS, на IPFS хеш которых при помощи cyberlink ссылается IPFS хеш файла (другой частицы) из поисковой строки.
